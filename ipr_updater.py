...
def local_version(prefix, suffix):
    """ 
...
def fetch_match(version):
    ...
    print('Downloading file match_complete-%i.xml.gz' % version)
...
def fetch_interpro(version):
    ...
    print('Downloading file interpro-%i.xml.gz' % version)
    urllib.request.urlretrieve('ftp://ftp.ebi.ac.uk/pub/databases/interpro/'
                               'interpro.xml.gz', 
...
def fetch_tsv(version, date):
    ...
    print('Downloading file uniprot-hproteome-%i-%s.tsv.gz' % (version, date))
...
def fetch_fasta(version, date):
    ...
    print('Downloading file uniprot-hproteome-%i-%s.fasta.gz' % (version, date))
    print('This should take about a minute.')
    queryline = ('http://www.uniprot.org/uniprot/'
                 '?query=reviewed:yes+AND+organism:9606+AND+proteome:up000005640'
...
# 2. Extract InterPro short names and parents.
#
